...
class _EditOrgProfileScreenState extends State<EditOrgProfileScreen> {
  ...
  void editThisOrg() async {
    ...
    if (_nameController.text.isEmpty ||
      ...
      return;
    } else if (_socMedController.text.isNotEmpty &&
        !Uri.tryParse(_socMedController.text.trim())!.hasAbsolutePath) {
      scaffoldMessenger.showSnackBar(
    ...
    try {
      ...
      scaffoldMessenger.showSnackBar(const SnackBar(
          content: Text('Successfully edited your organization profile')));
      goRouter.go('/orgHome');
    } catch (error) {
      scaffoldMessenger.showSnackBar(
  ...
  Widget _backButton() {
    return Row(children: [
      backToViewScreenButton(context,
          onPress: () => GoRouter.of(context).go('/orgHome'))
    ]);
  }
