...
class EditOwnOrgHeadScreen extends StatefulWidget {
  const EditOwnOrgHeadScreen({super.key});

  @override
  State<EditOwnOrgHeadScreen> createState() => _EditOwnOrgHeadScreenState();
}

enum RegisterStates { MEMBERSHIP, ORGANIZATION }

class _EditOwnOrgHeadScreenState extends State<EditOwnOrgHeadScreen> {
  ...
  void _editOrgHead() async {
    ...
    try {
      ...
      scaffoldMessenger.showSnackBar(
          const SnackBar(content: Text('Successfully edited your  account')));
      goRouter.go('/orgHome');
    } catch (error) {
      scaffoldMessenger.showSnackBar(
          SnackBar(content: Text('Error editing your account: $error')));
      setState(() {
        _isLoading = false;
      });
    }
